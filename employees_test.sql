...
select id, name, email, department from employee where email is not NULL and department is not NULL;
...
select id, name, age, 52-age as years_of_service_left from employee where age < 52 and age is not NULL;
...
else 'Can''t be determined for the employees with no value at salary'
end as pay_bucket
from employee;
...
select name,email from employee where email is NULL;

select count(*) as female_employees from employee where department ='Operations' and gender='Female';

select count(*) from employee where email is NULL;

select max(salary) from employee where department='Operations';

select avg(salary) from employee where gender='Female' and department='Testing';

select count(*) from employee where gender='Male' and department='SW';

select name,salary from employee where salary > 75000 order by age;

select sum(salary) from employee where department='HR' and age >= 27;

select name,age from employee where age is NOT NULL order by age desc ;

select name,date_of_joining from employee order by date_of_joining ;

select max(salary)-min(salary) from employee where gender='Female' and department='HR';

select max(salary) from employee where gender='Female' and department='HR';

select min(salary) from employee where gender='Female' and department='HR';

select avg(salary) from employee where date_of_joining > '02-09-2019' and department='Testing';

select name as Under_Payment from employee where department='Testing' and salary < 45000;

select department,count(*) from employee where salary < 45000 group by department;
...
select gender,max(age) ,min(salary),max(date_of_joining)  from employee group by gender; 

-- SELECT -> FROM -> WHERE -> GROUP BY -> ORDER BY

select department,count(*) as emp_cnt from employee where salary > 60000 group by department order by count(*);

select department,gender,count(*) from employee group by department,gender;

select department,count(*) as emp_cnt from employee group by department having emp_cnt > 27;
