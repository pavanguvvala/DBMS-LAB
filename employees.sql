...
INSERT INTO employees (id, name, age, gender, department, salary, date_of_joining, email) 
...
select count(*) from employees where age > (select avg(age) from employees);

select id, name, date_of_joining from employees where DATE_OF_JOINING = (select min(date_of_joining) from employees);

select id, name, date_of_joining from employees where DATE_OF_JOINING = (select max(date_of_joining) from employees);

select id, name, salary, department from employees where salary = (select max(salary) from employees);

select id, name, salary from employees where salary = (select max(salary) from employees where department='Testing');

select count(*) from employees where department = 'HR' and salary > (select avg(salary) from employees where department='SW');

select name,age from employees where age = (select min(age) from employees);

select id, name, department , salary from employees where salary = (select min(salary) from employees);

select name from employees where gender= 'Female' and department = 'Operations' and salary > (select max(salary) from employees where gender='Male' and department='Testing');

select  to_char(date_of_joining, 'YYYY') as year, count(*) from employees group by year;

select  to_char(date_of_joining, 'month') as month, count(*) from employees group by month order by month desc;


select count(*) from employees where gender='Male' and age < (select avg(age) from employees where gender='Female');

select name, salary from employees where (date_of_joining > '01-01-2021'and date_of_joining < '31-12-2021') and salary < (select avg(salary) from employees where (date_of_joining > '01-01-2020'and date_of_joining < '31-12-2020'));

SELECT ID,NAME,SALARY,department FROM EMPLOYEEs WHERE department = 'Testing' and SALARY = (SELECT MAX(SALARY) FROM EMPLOYEEs where department = 'Testing');

SELECT ID,NAME,DEPARTMENT,AGE FROM EMPLOYEEs WHERE (DEPARTMENT,AGE) IN (SELECT DEPARTMENT,MIN(AGE) FROM EMPLOYEEs where department is NOT NULL GROUP BY DEPARTMENT);

select id,name,salary,department from employees e where salary = (select max(SALARY) from employees where department=e.department);
...
-- fetch first 1 row only it is used to get first row only

with cte as 
( 
    select department , count(*) as cnt from employees where department is not null group by department
)
select department,cnt from cte where cnt =(select max(cnt) from cte); 
...
(
    select id,name,department,salary from employees where department='Testing'
)
select id, name,salary from hr_employees where salary > (select max(salary) from testing_employees);

select id,name,salary,dense_rank() over(order by salary desc) as rank  from employees where salary is not null;

select * from employee;

select sum(case when gender='Male' then salary when gender='Female' then -salary end) from employees;
