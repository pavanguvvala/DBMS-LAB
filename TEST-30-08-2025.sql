...
select * from enrollments e join courses c on e.course_id=c.c_id join students s on e.student_id=s.s_id where department_id=102 order by student_id;

select *   from course_mappings c join faculty f on c.faculty_id=f.f_id  order by f_id ;

select *  from course_mappings c join courses cc on c.course_id=cc.c_id where faculty_id=39 ;

select * from faculty f join departments d on f.department_id = d.d_id ;

select * from course_mappings where ROLE ='Secondary' order by FACULTY_ID;

select department_id,count(*) from students group by department_id order by count(*) desc;

select department_id,count(*) from students group by department_id order by department_id;

select c_name,count(*) from courses c join enrollments e on c.c_id=e.course_id group by c_name order by count(*) desc;

select  * from course_mappings where FACULTY_ID=20;

select COURSE_ID,count(*) from enrollments group by COURSE_ID order by count(*);

select STUDENT_ID,count(*) from enrollments group by student_id order by count(*) desc;

select location,count(*) from students s join departments d on s.department_id=d.d_id group by location;

select location,count(*) from departments group by location;

select sum(salary) from faculty where gender='Male' and DEPARTMENT_ID=101 ;

select * from faculty order by salary;

select department_id,avg(salary) from faculty group by department_id;

select department_id,count(*) from students group by department_id order by count(*) desc;

select COURSE_ID,count(*) from enrollments group by COURSE_ID order by COURSE_ID;

select * from students where name='Meena Sharma';

select to_char(DOJ,'YYYY') as dd,count(*) from faculty group by dd;

select name,DOB from students  order by dob desc;25/12/07
